...
3. Render reads [`render.yaml`](render.yaml) and provisions:
   - One web service running `uvicorn` (Python 3.11.10) that serves the built frontend.
...
4. The first deploy runs:
   ...
   ```
5. Fill in the secrets in the Render dashboard:
   ...
   `SESSION_SECRET` is auto-generated by Render. `DATABASE_URL` is wired by the blueprint.
